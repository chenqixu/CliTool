...
import com.cqx.cli.tool.bean.TimeVarBean;
...
public class TimeVarUtils {
    ...
    private Map<String, TimeVarBean> dateMap = new HashMap<>();
    ...
    public static void printlnDateHelp() {
        TimeVarUtils timeVarUtils = new TimeVarUtils();
        for (Map.Entry<String, TimeVarBean> entry : timeVarUtils.getDateMap().entrySet()) {
            logger.info("{} {}", entry.getValue().getDesc(), entry.getKey());
        }
    }
    ...
    public Map<String, TimeVarBean> getDateMap() {
        return dateMap;
    }
}
